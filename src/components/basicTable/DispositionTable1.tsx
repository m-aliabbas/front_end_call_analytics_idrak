...
export default function DispositionTable({
  ...
  useEffect(() => {
    const fetchStates = async () => {
      try {
        const response = await fetch('http://213.121.184.27:9000/get_new_states');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const url = `http://213.121.184.27:9000/get_all_logs/${selectedState}?start_index=${startIndex}&num_rows=${rowsPerPage}`;
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      try {
        const response = await fetch(`http://213.121.184.27:9000/get_counter_data/${selectedState}`);
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
        const json = await response.json();
